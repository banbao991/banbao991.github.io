...
let KEY = {
  ...
  delta_angle: PI2 / 360,
  delta_change_angle: PI2 / 360,
  MAX_DELTA_ANGLE: PI2 / 36,
  MIN_DELTA_ANGLE: PI2 / 360,
  OK_DELTA_ANGLE: PI2 / 120,
};
...
function main() {
  ...
  load_image("/resources/img/kits/qilabi_01.jpg").then(() => {
    ...
    // t_draw_pintu_border(context);
    // draw_pintu(context); // 集成在 change_pintu_size() 中了
...
function draw_after_event() {
  ...
  if (PINTU_CELL.mark_the_click_cell !== -1) {
    draw_a_cell(context, PINTU_CELL.mark_the_click_cell, null, false);
  }
  check_ok(context);
}
...
function detect_keydown(event) {
  ...
  //   console.log("B:", PINTU_CELL.now_translate[pos]);
  let k = event.key;
  if (k === "w") {
    PINTU_CELL.now_translate[pos].y -= KEY.delta;
  } else if (k === "a") {
    PINTU_CELL.now_translate[pos].x -= KEY.delta;
  } else if (k === "s") {
    PINTU_CELL.now_translate[pos].y += KEY.delta;
  } else if (k === "d") {
    PINTU_CELL.now_translate[pos].x += KEY.delta;
  } else if (k === "q") {
    PINTU_CELL.now_rotate[pos] -= KEY.delta_angle;
  } else if (k === "e") {
    PINTU_CELL.now_rotate[pos] += KEY.delta_angle;
  }
  // console.log(PINTU_CELL.now_translate[pos]);
  // (1) translate
...
function detect_click(event) {
  ...
  // 将选中的区域画到最上层
  if (PINTU_CELL.mark_the_click_cell !== -1) {
    draw_a_cell(context, PINTU_CELL.mark_the_click_cell, p, false);
  }
  check_ok(context);
}
 ...
 * 设置画布属性、input 属性等
 */
function init_the_window(canvas) {
  let init_h = $(window).height(),
    init_w = $(window).width();
  let h = init_h * 0.9,
    w = init_w * 0.9;
  if (2 * h < w) {
  ...
  CANVAS.height = h;
  // 注册事件
  canvas.addEventListener("click", detect_click);
  ...
  // 调节卡顿的按钮
  let kd = $("#kadun");
  let kd_num = $("#kadun_num").text("卡顿调节(" + KEY.MAX_WAIT_TIME + ")");
  ...
  for (let i = 0; i < size; ++i) {
    for (let j = 0; j < size; ++j) {
      ...
      PINTU_CELL.now_rotate.push(
        (Math.floor(Math.random() * 360) / 360) * PI2 // 保证最小变化为 1 度
        // PI2 / 8
      );
    }
  }
...
function clear_canvas(canvas) {
  canvas.width = canvas.width;
  canvas.height = canvas.height;
  draw_ref(canvas.getContext("2d"));
}

/** 绘制拼图的边框 */
function t_draw_pintu_border(context) {
  context.strokeStyle = PINTU.border_color;
  context.lineWidth = PINTU.border_line_width;
 ...
 * 检查是否完成拼图
 */
function check_ok(context) {
  let s = PINTU_CELL.dst_translate.length;
  for (let i = 0; i < s; ++i) {
    // translate
    if (
      Math.abs(PINTU_CELL.dst_translate[i].x - PINTU_CELL.now_translate[i].x) <=
        KEY.OK_DELTA &&
      Math.abs(PINTU_CELL.dst_translate[i].y - PINTU_CELL.now_translate[i].y) <=
        KEY.OK_DELTA
    ) {
      // rotate
      ...
      if (
        Math.abs(rot) < KEY.OK_DELTA_ANGLE ||
